...
try:
    ...
    print(f"Warning: Matrix inversion for FIM failed ({e}), using pseudo-inverse")
    hessian_inv = np.linalg.pinv(hessian)
cond_num = np.linalg.cond(hessian_inv)
if cond_num > 1e10:
    print(f"Warning: Covariance matrix is poorly conditioned (cond={cond_num:.2e}), adding regularization")
    hessian_inv += np.eye(hessian_inv.shape[0]) * 1e-6

godambe_samples = rng.multivariate_normal(
    mean=mle, cov=godambe_inv, size=args.posterior_samples,
)
pickle.dump(
    {"MLE": mle, "GIM": godambe_inv, "FIM": hessian_inv},
