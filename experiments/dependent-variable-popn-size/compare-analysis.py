...
def main():
    parser = argparse.ArgumentParser(
        description="Compare posterior distributions from multiple popgen-npe models",
...
Examples:
  ...
  %(prog)s --configs cnn.yaml rnn.yaml --labels "CNN" "RNN" --simulate --params 4.0 4.2 3.8 4.5 4.1 1.5e-8 --output comparison/
    ...
    input_group.add_argument(
        "--tree-sequence", help="Path to existing tree sequence file"
    )
    input_group.add_argument(
        "--simulate", action="store_true", help="Simulate new data"
    )

    # Parameters for existing data
    parser.add_argument(
        "--parameters", help="Path to parameters file (required with --tree-sequence)"
    )
    ...
    if args.simulate:
        ...
        if args.labels:
            ...
            labels = [
                os.path.splitext(os.path.basename(cfg))[0] for cfg in args.configs
            ]
        ...
        for config_path, label in zip(args.configs, labels):
            ...
            if "sequence_length" in sim_config:
                sim_cmd.extend(
                    ["--sequence-length", str(sim_config["sequence_length"])]
                )
    ...
    comparison_cmd = (
        [args.python, os.path.join(script_dir, "compare-posteriors.py"), "--configs"]
        + args.configs
        + [
            "--tree-sequence",
            tree_sequence,
