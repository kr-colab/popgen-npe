#!/usr/bin/env python3
"""
Create 6 panel plot with neural network posteriors and MSMC2 baseline.
...
def create_ribbon_plot_with_baselines(ax, posterior_samples, true_pop_sizes, change_times,
    ...
    times_with_one = np.concatenate([[1], np.array(change_times), [130000]])
...
def main():
    parser = argparse.ArgumentParser(
        description="Create 6 panel plot with neural network and MSMC2 baseline"
    )
    ...
    parser.add_argument(
        "--figsize", nargs=2, type=float, default=[15, 20],
        help="Figure size (width height)"
    )
    parser.add_argument(
        "--print-msmc2",
        action="store_true",
        help="Print MSMC2 time/Ne points before plotting"
    )
    ...
    for comp_dir in args.comparison_dirs:
        ...
        for part in parts:
            if part in ["medium","large","decline","expansion","bottleneck","zigzag"]:
    ...
    for scenario_idx, (results, scenario_name) in enumerate(zip(all_results, scenario_names)):
        ...
        if args.print_msmc2:
            if msmc2_data is None:
                print(f"\nMSMC2 points for scenario '{scenario_name}': <no data>")
            else:
                times_left = msmc2_data.get("times_left")
                times_right = msmc2_data.get("times_right")
                times = msmc2_data.get("times")
                ne = msmc2_data.get("ne")
                print(f"\nMSMC2 points for scenario '{scenario_name}':")
                if times_left is not None and times_right is not None and ne is not None:
                    for t_left, t_right, n in zip(times_left, times_right, ne):
                        print(f"  [{t_left:.6g}, {t_right:.6g}] -> Ne={n:.6g}")
                elif times is not None and ne is not None:
                    for t, n in zip(times, ne):
                        print(f"  t={t:.6g} -> Ne={n:.6g}")
                else:
                    print("  <missing times or Ne values>")
