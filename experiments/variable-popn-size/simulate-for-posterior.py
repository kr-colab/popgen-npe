...
def simulate_yri_ceu(params, config):
    ...
    graph = demes.Builder()
    graph.add_deme("ancestral", epochs=[dict(start_size=N_A, end_time=Tp + T)])
    graph.add_deme(
        "AMH",
        ancestors=["ancestral"],
    ...
    ts = msprime.sim_mutations(
        ts, rate=config["mutation_rate"], random_seed=config["seed"]
    )
...
def simulate_aratha_2epoch(params, config):
    ...
    demography.add_population_parameters_change(
        time=time_change, initial_size=N_A, population="pop"
    )
...
def simulate_variable_population_size(params, config):
    ...
    for i in range(1, len(pop_sizes)):
        demography.add_population_parameters_change(
            time=change_times[i - 1], initial_size=float(pop_sizes[i]), population="pop"
        )
...
def main():
    ...
    parser.add_argument(
        "--sequence-length", type=float, help="Sequence length (overrides default)"
    )
    parser.add_argument(
        "--mutation-rate", type=float, help="Mutation rate (overrides default)"
    )
    parser.add_argument(
        "--recombination-rate",
        type=float,
    ...
    if args.model == "VariablePopulationSize":
        ...
        param_names = [f"log10_N_{i+1}" for i in range(num_epochs)] + [
            "recombination_rate"
        ]
    else:
    ...
    params_data = {
        ...
        "simulator_config": {"class_name": args.model},
    }
