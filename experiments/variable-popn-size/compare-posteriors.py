...
Examples:
  ...
  %(prog)s --configs model1.yaml model2.yaml --labels "CNN" "RNN" --tree-sequence data.trees --parameters params.yaml --output comparison/
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to tree sequence file (.trees)",
    )
    parser.add_argument(
    ...
    print(f"Loaded tree sequence: {ts.num_samples} samples, {ts.sequence_length} bp")
    ...
    # # Get prior bounds
    # if "prior_bounds" in params_data:
    #     prior_low = np.array(params_data["prior_bounds"]["low"])
    #     prior_high = np.array(params_data["prior_bounds"]["high"])
    # else:
    #     prior_low = np.array([2.0] * n_epochs + [1e-9])
    #     prior_high = np.array([6.0] * n_epochs + [1e-7])
    ...
    for i, config_path in enumerate(args.configs):
        ...
        # Set label
        if args.labels:
            model_data["label"] = args.labels[i]
        else:
            model_data["label"] = model_data["model_name"]
